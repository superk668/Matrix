...
def nrandom(dim):
    lst = [[random.randint(-100, 100) for c in range(dim[1])] for r in range(dim[0])]
    return Matrix(lst)

def nrandom_like(matrix):
    lst = [[random.randint(-100, 100) for c in range(matrix.dim[1])] for r in range(matrix.dim[0])]
    return Matrix(lst)
